    ...
    public class AdminRequestsCountSpec : BaseSpecifications<PrescriptionRequestEntity, int>
    {
        public AdminRequestsCountSpec(PrescriptionRequestQueryParams p)
            : base(r =>
                ...
                || (r.MedicineName != null && r.MedicineName.ToLower().Contains(p.Search.ToLower()))
                || (r.PatientProfile != null && r.PatientProfile.ApplicationUser != null && r.PatientProfile.ApplicationUser.PhoneNumber != null && r.PatientProfile.ApplicationUser.PhoneNumber.Contains(p.Search)))
            )
        {
        }
    }
